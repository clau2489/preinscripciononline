<div class="container">
  <div class="row p2 text-center">
    <div class="offset-md-3 col-md-6">
      <br>
      <a href="https://www.facebook.com/modernizacionmmp"><img src="assets/img/modernizacion.png" width="70%" class="p1"></a>
    </div>
  </div>  
</div>
<footer class="footer mastheads d-flex text-center" style="background-color: #82c024; border-bottom-style: ridge; border-color: #90ff3b;">
  <div class="container p2">
    <div class="row">
      <div class="col-md-12">
        <p class="copyright">Modernización e Innovación - &copy;marcospazdigital - Versión 1.0 - Municipio de Marcos Paz</p>
      </div>
    </div>
    <div class="row">
      <div class="col-md-12">
        <a href="https://www.facebook.com/modernizacionmarcospaz"><i class="fa fa-facebook-square icon" style="font-size: 28px"></i></a>
        <a href=""><i class="fa fa-twitter-square icon" style="font-size: 28px"></i></a>
        <a href="https://www.instagram.com/modernizacionmmp/"><i class="fa fa-instagram icon" style="font-size: 28px"></i></a>
        <a href=""><i class="fa fa-youtube-square icon" style="font-size: 28px"></i></a>         
      </div>
    </div>  
  </div>
</footer>